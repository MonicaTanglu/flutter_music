...
import 'package:flutter_music/common/global.dart';
...
class FindPageState extends State with AutomaticKeepAliveClientMixin {
  ...
  /* dart使用future对象来表示异步操作结果。要想使用futures可以使用async和await使用
  **
   */
  ...
  Widget _itemBuilder(int index) {
    ...
    return SizedBox(
      ...
      child: new Padding(
        ...
        child: ImageBlock(
            {'picUrl': item['album']['picUrl'], 'name': item['name']}),
      ),
    );
  }
                    ...
                    child: Align(
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          box,
                          Text(item['name'], style: TextStyle(fontSize: 12.0))
                        ],
                      ),
                    ),
                  );
